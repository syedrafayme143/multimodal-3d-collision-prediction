...
def get_test_scene_indices(config_path: str) -> set:
    """
    Fetches the split scene indices from create_splits.py using positional tuple unpacking.
    Passes the custom config_path downstream to ensure split generation respects any
    user-specified configurations (e.g. experiment YAMLs) instead of defaults.
    """
    logger.info("Random scene-level splitting with fixed seed is active.")
    
    # Pass config_path downstream so any custom split configurations are correctly loaded
    _, _, test_scenes_list = generate_scene_splits(config_path)
    test_scene_indices = set(test_scenes_list)
    
    if not test_scene_indices:
        raise ValueError(
            "generate_scene_splits() returned no scenes for the test split."
        )
    logger.info("Loaded %d scene indices for the test split.", len(test_scene_indices))
    return test_scene_indices
...
def iter_test_samples(nusc: NuScenes, test_scene_indices: set):
    """
    ...
    NOTE: Index-based scene resolution is a temporary solution ideal for nuScenes-mini.
    For production environments, you should rewrite the splitter to save and return 
    real scene names (e.g. 'scene-0061') or unique tokens to ensure order-independence.
    """
    for idx, scene in enumerate(nusc.scene, start=1):
        if idx in test_scene_indices:
...
def run(config_path: str):
    ...
    # --- Data & splits -----------------------------------------------------
    # FIX: Pass the raw config_path directly so splits are generated using the active configuration
    test_scene_indices = get_test_scene_indices(config_path)
    ...
    # --- Inference loop --------------------------------------------------
    samples = list(iter_test_samples(nusc, test_scene_indices))
